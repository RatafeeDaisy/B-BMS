...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 7% 0","alignItems":"center","background":"none","display":"flex","width":"100%","justifyContent":"flex-end","height":"auto"}'>
      ...
      <el-form-item :style='{"margin":"0 4px 0 0","justifyContent":"center","display":"flex"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>房间状态</div>
        <el-select v-model="formSearch.fangjianzhuangtai" placeholder="请选择房间状态" :clearable="true">
          <el-option v-for="(item, index) in fangjianzhuangtaiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('fangjianxinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#1e8892","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/fangjianxinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
        this.$http.get('option/fangjianleixing/fangjianleixing').then(res => {
          if (res.data.code == 0) {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.indexQueryCondition) {
          searchWhere.fangjianleixing = '%' + this.indexQueryCondition + '%';
        ...
        if (this.formSearch.fangjianzhuangtai != '') searchWhere.fangjianzhuangtai = this.formSearch.fangjianzhuangtai;
        if (this.curFenlei != '全部') searchWhere.fangjianleixing = this.curFenlei;
        this.$http.get('fangjianxinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/fangjianxinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
